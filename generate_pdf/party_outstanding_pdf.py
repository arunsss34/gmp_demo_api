...
from generate_pdf.outstanding_pdf import file_to_base64
import uuid
...
# Party info in Party Outstanding Pdfs
def get_outstanding_overview_pdf(request, decoded):  # V1 -----------> basic and full all Party outstanding api starts here
...
def basic_pdf_format(datas, outstanding, main_table, decoded, report_name, basic_agent_data):
    try:
        ...
        lucky_yarns_agents = [agent_data for agent_data in datas if agent_data['Company'] ==
                              'Lucky Yarn Tex India Private Limited']
        lucky_yarns_total = 0

        for customer_data in lucky_yarns_agents:
            customer_id = customer_data['CustomerID']
            data = get_agent_basic_data(customer_id, 'OutstandingLuckyYarnsSpinning', basic_agent_data)
            ...
            text = f"Party: {customer_data['CustomerName']}"
            ...
            for row in data:
                pdf.cell(95, 10, str(row['AgentName']), 1, 0, 'C')
            ...
            pdf.cell(95, 10, "Party Total", 1, 0, 'C')
        ...
        for customer_data in lucky_weaves_agents:
            customer_id = customer_data['CustomerID']
            data = get_agent_basic_data(customer_id, 'OutstandingLuckyWeavesSpinning', basic_agent_data)
...
def get_agent_basic_data(customer_id, main_table, procedure_name):
    try:
        qry = '{call Reporting.' + str(procedure_name) + '(?,?)}'
        res, k = py_connection.call_prop_col(qry, (customer_id, main_table))
        lst = []
        if res and len(res) > 0:
            for row in res:
                view_data = dict(zip(k, row))
    ...
    try:
        ...
        if agent_details and len(agent_details) > 0:
            for row in agent_details:
                ...
                view_data['party_info'] = get_party_info(view_data['CustomerID'], party_info, party_details, view_data['Company'])
                lst.append(view_data)
            return lst
        else:
            return lst
...
def get_party_info(customer_id, procedure_name, party_details, Company):
    try:
        qry = '{call Reporting.' + str(procedure_name) + '(?,?,?,?,?)}'
        res, k = py_connection.call_prop1(qry, (customer_id, 0, 0, 1, Company))
        lst = []
        if res and len(res) > 0:
            for row in res:
                view_data = dict(zip(k, row))
                view_data['party_details'] = get_party_details(customer_id, view_data['AgentID'], party_details, Company)
                lst.append(view_data)
            return lst
        else:
            return lst
...
def get_party_details(agent_id, customer_id, procedure_name, Company):
    try:
        qry = '{call Reporting.' + str(procedure_name) + '(?,?,?,?,?,?)}'
        res, k = py_connection.call_prop1(qry, (customer_id, agent_id, 0, 0, 1, Company))
        lst = []
        if res and len(res) > 0:
            for row in res:
                view_data = dict(zip(k, row))
    ...
    try:
        ...
        for company in ['Lucky Yarn Tex India Private Limited', 'Lucky Weaves India Private Limited']:
            ...
            for agent_data in company_agents:
                if pdf.get_y() > 250:  # Adjust this value as per your page's layout
                    pdf.add_page()
                customer_name = agent_data['CustomerName']
                agent_total = 0  # Initialize agent total

                # Centered agent name
                pdf.set_font("Arial", "B", 11)
                text = f"CustomerName: {customer_name}"
                text_width = pdf.get_string_width(text)
                page_width = pdf.w - 2 * pdf.l_margin
                x_position = (page_width - text_width) / 2
                pdf.set_x(x_position)
                ...
                for agent in agent_data['party_info']:
                    agent_name = agent['AgentName']
                    party_total = 0  # Initialize party total

                    # Display customer name
                    pdf.set_font("Arial", "B", 9)
                    pdf.cell(0, 6, f"AgentName: {agent_name}", ln=True)
                    # pdf.ln(5)
                    ...
                    for idx, detail in enumerate(agent['party_details'], start=1):
                ...
                # Agent Total
                pdf.set_font("Arial", "B", 10)
                pdf.cell(0, 6, f"Party Total: {agent_total:.2f}", ln=True, align='R')
                pdf.ln(10)
...
# Agent info in Party Outstanding Pdfs
def get_outstanding_party_overview_pdf(request, decoded):  # V1 -----------> basic and full all party api starts here
    config_details = get_table_config_details(request, decoded)
    if config_details and len(config_details) > 0:
...
def get_party_outstanding_total(procedure_name, request):  # Fn for getting Outstanding total, no.of customers, date
    try:
        ...
        for agent in request["customer_id"]:
            ...
            res, k = py_connection.call_prop_col(qry, (customer_id, 0, 0, 1, company_name))
...
def basic_party_pdf_format(datas, outstanding, main_table, decoded, request):
    try:
        ...
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 10, f"Party: {datas[0]['CustomerName']}", ln=True, align='C')
        pdf.ln(3)
        ...
        for customer in customers:
            if pdf.get_y() > 250:  # Adjust this value as per your page's layout
                pdf.add_page()
            customer_id = customer['id']  # Get the agent's ID
            company = customer['Company']
            ...
            pdf.cell(company_text_width, 8, company, ln=True, align='C')  # Centered company name

            pdf.ln(2)  # Add a small line break

            # Retrieve agent data
            data = get_agent_data1(customer_id, 'OutstandingLuckyYarnsSpinning' if company == 'Lucky Yarn Tex India Private Limited'
            ...
            for row in data:
                ...
                if agent_name not in agent:
                    agent[agent_name] = []
                agent[agent_name].append(row)

            # Print each customer's data
            for agent_name, agent_data in agent.items():
                if pdf.get_y() > 250:  # Adjust this value as per your page's layout
                    pdf.add_page()
                pdf.cell(0, 6, f"AgentName: {agent_name}", ln=True)
                ...
                pdf.set_font("Arial", "B", 8)
                pdf.cell(76, 10, 'Agent Total', 1, 0, 'C')
        ...
        # Add agent total
        pdf.set_font("Arial", "B", 10)
        pdf.cell(100, 10, "Party Total", 1, 0, 'C')
...
def get_agent_data1(customer_id, main_table):  # Fn to get the required datas
    try:
        qry =("SELECT AgentName, AgentID, CustomerName,CustomerID, BalanceAmount,FORMAT(CAST(InvoiceDate AS DATE), 'dd-MM-yyyy') as InvoiceDate,InvoiceNo,"
              "FORMAT(CAST(InvoiceDate AS DATE), 'dd-MM-yyyy') as DueDate,PendingDays  FROM dbo." + str(main_table) +
              " where CustomerID = " + str(customer_id) + " Group BY AgentID, AgentName,CustomerName,CustomerID,BalanceAmount,InvoiceDate,InvoiceNo,"
              "DueDate,PendingDays  order by CustomerName asc , cast(InvoiceDate as date) asc")
        res, k = py_connection.get_result_col(qry)
        lst = []
        if res and len(res) > 0:
    ...
    try:
        ...
        for customer in request['customer_id']:
            ...
            if res and len(res) > 0:
                for row in res:
                    view_data = dict(zip(k, row))
                    view_data['party_details'] = get_each_party_details(view_data['AgentID'], customer_id, party_details, company)
                    lst.append(view_data)
        return lst
    except Exception as e:
        print(str(e))
...
def detailed_party_pdf_format(datas, outstanding, decoded):
    try:
        ...
        for agent in datas:
            ...
            for detail in agent['party_details']:
                invoice_no = detail['InvoiceNo']
                invoice_date = detail['InvoiceDate']
                terms = detail['Terms']
            ...
            if agent['party_details']:
                pdf.set_font("Arial", "B", 8)
                tds_text = f"TDS: {agent['party_details'][-1].get('TDSBalanceAmount', 0.00):.2f}"
                party_total_text = f"Agent Total: {party_total:.2f}"
...
# Party Details In Party Outstanding
...
def generate_party_details_pdf(res, request, outstanding, main_table, decoded):
    try:
        ...
        customer_name = res[0]['CustomerName']  # Agent name is common across all customers
        company = request['company']
        page_width = pdf.w - 2 * pdf.l_margin

        # Center agent name
        pdf.set_font("Arial", "B", 13)
        agent_text = f"Party: {customer_name}"
        ...
        for entry in res:
            ...
            if agent_name not in agent_group:
                agent_group[agent_name] = []
            agent_group[agent_name].append(entry)

        agent_total = 0  # Initialize agent total
        for agent_name, details in agent_group.items():
            party_total = 0  # Initialize party total

            # Display customer name
            pdf.set_font("Arial", "B", 9)
            pdf.cell(0, 6, f"Agent: {agent_name}", ln=True)
